...
class TrackingService : Service() {
        ...
        alertListener = object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                for (child in snapshot.children) {
                    ...
                    // Only send SMS once per SOS trigger (prevent spam if service restarts)
                    if (active && (System.currentTimeMillis() - time) < 60000 && time > lastSosSmsTime) {
                        lastSosSmsTime = time
                        if (ContextCompat.checkSelfPermission(this@TrackingService, android.Manifest.permission.SEND_SMS) == android.content.pm.PackageManager.PERMISSION_GRANTED) {
                            sendEmergencySms(lastKnownLat, lastKnownLng)
                        } else {
                            Log.e("TrackingService", "SEND_SMS permission not granted!")
                        }
                    }
                }
            }
    ...
    private fun sendEmergencySms(lat: Double, lng: Double) {
        val sharedPrefs = getSharedPreferences("AshGoPrefs", Context.MODE_PRIVATE)
        val contactsString = sharedPrefs.getString("emergency_contacts", "") ?: ""
        if (contactsString.isBlank()) return

        val message = "🚨 EMERGENCY SOS 🚨 Ash has triggered an SOS! Last known location: https://maps.google.com/?q=$lat,$lng"
        ...
        for (contact in contacts) {
            val phone = contact.trim()
            if (phone.isNotEmpty()) {
                try {
                    smsManager?.sendTextMessage(phone, null, message, null, null)
                    Log.i("TrackingService", "Emergency SMS sent to $phone")
                } catch (e: Exception) {
                    Log.e("TrackingService", "Failed to send SMS to $phone", e)
                }
            }
        }
    }
